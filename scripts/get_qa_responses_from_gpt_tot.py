...
def main(
    ...
    old_api_key = "YOUR_GPT_API_KEY_2" 
    old_api_key_mine = "YOUR_GPT_API_KEY_2" 
    api_counter = 0
    ...
    with xopen(input_path) as fin:
        for line in tqdm(list(fin)):
            ...
            if closedbook:
                _prompt = get_closedbook_qa_prompt(question, gpt=True, tot_prompting=tot_prompting)
                prompt = f"Write a high-quality answer for the given question.##;##{_prompt}"
            else:
    ...
    with xopen(output_path, "a") as f:
        for i in tqdm(range(len(prompts))):
            ...
            try:
                tot_response = openai.ChatCompletion.create(
                    model=model_name,
                    messages=[
                ...
                tot_response = tot_response['choices'][0]['message']['content']
                system_prompt, user_prompt = get_reduce_prompt(example["question"], tot_response, tot_prompting=True)
                ...
                tot_response = completion_with_backoff(
                    model=model_name,
                    messages=[
                        {"role": "system", "content": system_prompt},
                        {"role": "user", "content": user_prompt}
                ...
                tot_response = tot_response['choices'][0]['message']['content']
                system_prompt, user_prompt = get_reduce_prompt(question=example["question"], map_output=tot_response, tot_prompting=True)
    ...
    with open("missing_responses_gpt.txt", "w") as f:
